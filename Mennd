...
async def buy_callback(callback: types.CallbackQuery):
    # Сразу подтверждаем получение callback'а
    await callback.answer()
        ...
        try:
            async with db_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    # Вставляем запись о бизнесе (без ON CONFLICT)
                    await conn.execute(
...
async def add_player_to_game(game_id: str, user_id: int, username: str):
    async with db_pool.acquire() as conn:
        # Используем SELECT FOR UPDATE, чтобы заблокировать строку игры
        async with conn.transaction():
...
async def start_game(game_id: str):
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            # Блокируем игру и всех её игроков
            game = await conn.fetchrow("SELECT * FROM multiplayer_games WHERE game_id=$1 AND status='waiting' FOR UPDATE", game_id)
            if not game:
                raise ValueError("Игра не найдена или уже началась")
            ...
            # Проверяем баланс каждого игрока и списываем ставку
...
async def check_giveaways():
    while True:
        try:
            ...
            now = datetime.now()
            async with db_pool.acquire() as conn:
                # Находим активные розыгрыши, у которых end_date <= now
                expired = await conn.fetch("""
                ...
                for gw in expired:
                    ...
                    # Выбираем победителей (без повторений, если участников меньше, то берём всех)
                    if len(participant_ids) <= winners_count:
                        winners = participant_ids
                    else:
                        winners = random.sample(participant_ids, winners_count)

                    winners_str = ", ".join(str(uid) for uid in winners)
                    await conn.execute(
                        "UPDATE giveaways SET status='completed', winners_list=$1 WHERE id=$2",
                        winners_str, gw_id
                    )
                    ...
                    # Уведомление в чаты, если нужно
                    if await get_setting("chat_notify_giveaway") == "1":
                        await notify_chats(f"🏁 Розыгрыш #{gw_id} завершён! Победители: {winners_str}")
        except Exception as e:
            logging.error(f"Error in check_giveaways: {e}", exc_info=True)
            await asyncio.sleep(60)
...
if __name__ == '__main__':
    ...
    loop.create_task(check_giveaways())  # добавлена задача для розыгрышей
